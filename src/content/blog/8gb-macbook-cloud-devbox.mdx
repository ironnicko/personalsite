...
The obvious move is "buy a 32GB MacBook." But a laptop upgrade is real money out of pocket, and — conveniently — **my startup had GCP credits sitting around doing nothing.** So instead of buying hardware I couldn't justify, I spent credits I already had.
...
### 5. Sessions as a markdown file

Long-running remote agents have a memory problem: *I* forget what I asked for yesterday, and a fresh agent has no idea either. So each project gets a plain `SESSIONS.md` that acts as a running handoff log:

```markdown
# Sessions

## 2026-07-19 — auth refactor
- Done: swapped session store to Redis, tests green
- Next: migrate the /login handler, then delete the old JWT path
- Watch out: staging still points at the old table
```

At the start of a session Claude reads it; at the end it appends what changed and what's next. Where `CLAUDE.md` holds the *standing rules* for a repo, `SESSIONS.md` holds the *running state*. Reattach tmux the next morning and the context is right there — for me and for the agent.
